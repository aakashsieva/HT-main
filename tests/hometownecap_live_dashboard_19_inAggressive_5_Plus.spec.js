...
test('In Aggressive (5+ Days)', async ({ page }) => {
 ...
 */
  ...
  const inAggressive = page.locator('.divVehicleInAggressiveMode .btn5daysPlus').first();
  ...
  // Unit column
  await page.locator('.secInAggDevice .dataTables_scrollHead table th:has-text("Unit")').click();
  await page.waitForTimeout(2000);
  await page.locator('.secInAggDevice .dataTables_scrollHead table th:has-text("Unit")').click();
  await page.waitForTimeout(2000);

  // Last Activity column
  await page.locator('.secInAggDevice .dataTables_scrollHead table th:has-text("Last Activity")').click();
  await page.waitForTimeout(2000);
  await page.locator('.secInAggDevice .dataTables_scrollHead table th:has-text("Last Activity")').click();
  await page.waitForTimeout(2000);

  // Location column
  const mostRecent = page.locator('.secInAggDevice .dataTables_scrollHead table th')
  ...
  // Details column
  await page.locator('.secInAggDevice .dataTables_scrollHead table th:has-text("Battery")').click();
  await page.waitForTimeout(2000);
  await page.locator('.secInAggDevice .dataTables_scrollHead table th:has-text("Battery")').click();
  await page.waitForTimeout(2000);
  ...
  await searchInput.fill('861492060168561');
