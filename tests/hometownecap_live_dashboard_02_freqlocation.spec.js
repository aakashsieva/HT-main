...
test('Frequent Location', async ({ page }) => {
  ...
  test.setTimeout(600000); // 2 minutes for the entire test
  ...
  await page.locator('.secTrackFreqStop #tblTrackFreqStopReport th:has-text("Most Frequent Locations")').click();
  await page.waitForTimeout(2000);
  await page.locator('.secTrackFreqStop #tblTrackFreqStopReport th:has-text("Most Frequent Locations")').click();
  await page.waitForTimeout(2000);

  // # of times column  
  await page.locator('.secTrackFreqStop #tblTrackFreqStopReport th:has-text("# of times")').click();
  await page.waitForTimeout(2000);
  await page.locator('.secTrackFreqStop #tblTrackFreqStopReport th:has-text("# of times")').click();
  await page.waitForTimeout(2000);
 
  // Most Recent
  const mostRecent = page.locator('#tblTrackFreqStopReport th')
  ...
  // 2nd Most Recent
  const secondMostRecent = page.locator('#tblTrackFreqStopReport th')
  ...
  // 3rd Most Recent
  const thirdMostRecent = page.locator('#tblTrackFreqStopReport th')
